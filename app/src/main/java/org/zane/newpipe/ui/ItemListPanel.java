...
public class ItemListPanel<T extends InfoItem> extends JPanel {
    ...
    public ItemListPanel(
        ...
        Thread.startVirtualThread(() -> {
            try {
                itp = listExtractor.getInitialPage();
                showPage();
    ...
    protected ItemListPanel(MainViewPort mainViewPort) {
    ...
    public void onPreBtnPressed() {
        try {
            if (pageStack.isEmpty()) {
                ...
                itp = listExtractor.getPage(prePage);
                showPage();
                SwingUtilities.invokeLater(() -> {
    ...
    private void onNextBtnPressed() {
        try {
            ...
            itp = listExtractor.getPage(nextPage);
            mainViewPort.setViewPosition(new Point(0, 0));
            showPage();
            if (currentPage != null) {
    ...
    protected void setListExtractor(ListExtractor<T> listExtractor)
